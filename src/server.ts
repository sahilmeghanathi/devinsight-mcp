...
const tools: Tool[] = [
  {
    name: "analyze_error",
    description:
      "Analyzes a JavaScript/TypeScript error message and provides root cause, fix suggestions, and code examples. Covers 100+ error patterns including TypeError, ReferenceError, and more.",
    inputSchema: {
  ...
  },
  {
    ...
    description:
      "Analyzes multiple error messages at once and returns insights for each error.",
    inputSchema: {
      type: "object" as const,
      properties: {
        errors: {
  ...
  },
  {
    ...
    description:
      "Scans the TypeScript/JavaScript project to find unused functions and potentially unused files. Helps identify dead code that can be safely removed.",
    inputSchema: {
      type: "object" as const,
      properties: {
        confidence_filter: {
    ...
    case "find_unused_code": {
      ...
      try {
        ...
        if (confidence_filter !== "any") {
          filteredFunctions = result.unusedFunctions.filter(
            (f: any) => f.confidence === confidence_filter
          );
        }
...
async function main() {
  ...
  server.setRequestHandler(CallToolRequestSchema, async (request: any) => {
    ...
    const result = await processToolCall(name, args as Record<string, any>);
